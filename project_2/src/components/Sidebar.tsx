...
const navSections: { title: string; items: NavItem[] }[] = [
  {
    ...
    items: [
      ...
      { icon: KanbanSquare, label: 'Task Board', view: 'tasks' },
      { icon: CalendarDays, label: 'Schedule', view: 'schedule' },
      { icon: Timer, label: 'Pomodoro Sessions', view: 'pomodoro', badge: '3' },
    ],
  },
  {
    title: 'Tools',
    items: [
      ...
      { icon: Sparkles, label: 'AI Insights', view: 'ai-insights', badge: 'New' },
    ],
  },
  {
    title: 'Other',
    items: [
      { icon: Bell, label: 'Notification', view: 'notifications', dot: true },
...
function SidebarContent({ activeView, onNavigate, user, onClose }: SidebarProps & { onClose?: () => void }) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 pb-6 shrink-0">
        <button
          ...
          style={{
            background: activeView === 'profile' ? '#DDF3DF' : '#F4FAF4',
            border: activeView === 'profile' ? '1px solid rgba(95,175,110,0.4)' : '1px solid transparent',
          }}
        >
          <div
          ...
          <div className="flex-1 min-w-0 text-left">
            <p className="text-sm font-semibold truncate" style={{ color: '#243024' }}>{user.name}</p>
            <p className="text-xs truncate" style={{ color: '#5F6E5F' }}>Streak: {user.streak} ngày</p>
          </div>
...
export function Sidebar({ activeView, onNavigate, user }: SidebarProps) {
  ...
  return (
    <>
      ...
      >
        <SidebarContent activeView={activeView} onNavigate={onNavigate} user={user} />
      </aside>
      ...
      >
        {[
          { icon: LayoutDashboard, view: 'dashboard' as ViewType, label: 'Home' },
          { icon: KanbanSquare, view: 'tasks' as ViewType, label: 'Tasks' },
          { icon: Timer, view: 'pomodoro' as ViewType, label: 'Focus' },
          { icon: BarChart2, view: 'analytics' as ViewType, label: 'Stats' },
        ].map((item) => {
          const isActive = activeView === item.view;
          return (
            <button
