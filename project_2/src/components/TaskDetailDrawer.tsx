...
import {
  X, CheckCircle2, Circle, Trash2, Plus, ChevronUp, ChevronDown,
  Clock, Calendar, Timer, AlertCircle, Pencil, Check,
} from 'lucide-react';
import type { Task, TaskType, Priority, Subtask } from '../mockData';
...
export function TaskDetailDrawer({ task, open, onClose, onSave, onDelete }: Props) {
  ...
  const updateSubtask = (id: string, field: 'title' | 'minutes', value: string | number) =>
    setDraft((d) => d ? {
      ...d,
  ...
  const addSubtask = () =>
    setDraft((d) => d ? {
      ...d,
      subtasks: [...d.subtasks, { id: `new-${Date.now()}`, title: '', done: false, minutes: 15 }],
    } : null);
  ...
  const inputFieldCls = 'w-full px-4 py-2.5 rounded-xl text-sm outline-none transition-all duration-200 bg-[#F4FAF4]';
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl" style={{ background: '#F4FAF4', border: '1.5px solid #E8F5E8' }}>
                    ...
                    <input
                      ...
                      onChange={(e) => setDraft((d) => d ? { ...d, deadline: e.target.value } : null)}
                      placeholder="vd: Hôm nay, 17:00"
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl" style={{ background: '#F4FAF4', border: '1.5px solid #E8F5E8' }}>
                    ...
                    <input
                      ...
                      onChange={(e) => setDraft((d) => d ? { ...d, fixedTime: e.target.value } : null)}
                      placeholder="vd: Thứ 5, 10:00–11:00"
                      className="flex-1 bg-transparent outline-none text-sm"
                      style={{ color: '#243024' }}
                    />
                  </div>
