...
import {
  ...
  ChevronUp, ChevronDown, AlertCircle, Timer,
} from 'lucide-react';
import type { TaskType, Priority, Subtask } from '../mockData';
...
/* ─── AI mock subtasks per keyword ─── */
const AI_SUBTASKS: Subtask[] = [
  { id: 'ai-1', title: 'Tìm tài liệu tham khảo', done: false },
  { id: 'ai-2', title: 'Lập dàn ý chi tiết', done: false },
  { id: 'ai-3', title: 'Thực hiện bước đầu tiên', done: false },
  { id: 'ai-4', title: 'Kiểm tra và chỉnh sửa', done: false },
  { id: 'ai-5', title: 'Hoàn thiện và nộp', done: false },
];

const AI_MINUTES = [30, 20, 45, 25, 15];
...
export function CreateTaskDrawer({ open, onClose, onSave }: Props) {
  ...
  const [subtasks, setSubtasks] = useState<(Subtask & { minutes: number })[]>([]);
  ...
  const reset = () => {
    setTitle(''); setType('flexible'); setPriority('high');
    setDeadline(''); setDate(''); setStartTime(''); setEndTime('');
    setSubtasks([]); setAiLoading(false); setTypeVisible(false);
  };
  ...
  const handleAI = () => {
    ...
    setTimeout(() => {
      setSubtasks(
        AI_SUBTASKS.map((s, i) => ({ ...s, id: `ai-${Date.now()}-${i}`, minutes: AI_MINUTES[i] }))
      );
      setAiLoading(false);
    }, 1600);
  };
  ...
  const addSubtask = () =>
    setSubtasks((p) => [...p, { id: `m-${Date.now()}`, title: '', done: false, minutes: 15 }]);

  const updateSubtask = (id: string, field: 'title' | 'minutes', val: string | number) =>
  ...
  const totalMin = subtasks.reduce((s, t) => s + (Number(t.minutes) || 0), 0);

  /* ─── Input style helper ─── */
  const inputCls = 'w-full px-4 py-2.5 rounded-xl text-sm outline-none transition-all duration-200 bg-[#F4FAF4] text-[#243024]';
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-3">
              {([
                { val: 'high' as Priority, label: 'High', bg: '#F6D8C7', text: '#C1644C' },
                { val: 'low' as Priority, label: 'Low', bg: '#F7E7A8', text: '#B8860B' },
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          <input
                            ...
                            value={sub.minutes}
                            min={1}
                            onChange={(e) => updateSubtask(sub.id, 'minutes', Number(e.target.value))}
